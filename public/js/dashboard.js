...
const Dashboard = {
    ...
    async _renderFacultyTodayClasses() {
        ...
        const allEntries = await dataStore.getFacultyTimetable(this.session.id);
        const classesTodayEl = document.getElementById('classes-today');

        // Determine today's day code (Mon/Tue/Wed/Thu/Fri)
        const dayNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
        const todayCode = dayNames[new Date().getDay()];

        if (todayCode === 'Sun' || todayCode === 'Sat') {
            if (classesTodayEl) classesTodayEl.textContent = '0';
            container.innerHTML = '<p style="opacity:0.7;font-size:0.9rem;color:var(--text-color);">No classes today (Weekend) 🎉</p>';
            return;
        }

        // Filter to today only, then deduplicate by time (show each time slot once)
        const todayEntries = allEntries.filter(e => e.day === todayCode);

        // Group by time slot, collect all sections for that slot
        const byTime = {};
        ...
        if (classesTodayEl) classesTodayEl.textContent = sortedTimes.length;

        if (sortedTimes.length > 0) {
            container.innerHTML = sortedTimes.map(time => {
                const entries = byTime[time];
                const labels = entries.map(e => `${e.subject} <span style="opacity:0.7;">(Sec ${e.section})</span>`).join(', ');
                const rooms  = [...new Set(entries.map(e => e.room || e.location || ''))].join(' / ');
                return `
                    <div class="event-item" style="border-left-color:var(--text-color);">
                        <div class="event-date">${time}</div>
                        <h4>${labels}</h4>
                        <p style="font-size:0.9rem;opacity:0.8;"><i class="fas fa-map-marker-alt"></i> ${rooms}</p>
                    </div>`;
            }).join('');
        } else {
