...
class DataStore {
    ...
    // TIMETABLE
    // ============================================================
    async getTimetable(semester, section) {
        try {
            const sec = section ? `?section=${encodeURIComponent(section)}` : '';
            return await api(`/timetable/${encodeURIComponent(semester)}${sec}`);
        } catch {
            return null;
        }
    }
    ...
    async getFacultyTimetable(facultyId) {
        try {
            ...
            return await api(`/faculty-timetable${q}`);
        } catch {
            return [];
        }
    }
